...
class PluginBehaviorsITILSolution
{
    ...
    public static function beforeAdd(ITILSolution $soluce)
    {
        ...
        $ticket = new Ticket();
        if (
            $ticket->getFromDB($soluce->input['items_id'])
            ...
            if (
                $config->getField('is_ticketrealtime_mandatory')
            ...
            //            }
            if (
                $config->getField('is_ticketcategory_mandatory')
            ...
            if (
                $config->getField('is_tickettech_mandatory')
            ...
            if (
                $config->getField('is_tickettechgroup_mandatory')
            ...
            if (
                $config->getField('is_ticketlocation_mandatory')
        ...
        $problem = new Problem();
        if (
            $problem->getFromDB($soluce->input['items_id'])
            && ($soluce->input['itemtype'] == 'Problem')
        ) {
            if (
                $config->getField('is_problemsolutiontype_mandatory')
                && empty($soluce->input['solutiontypes_id'])
            ) {
                $soluce->input = false;
                Session::addMessageAfterRedirect(
        ...
        $change = new Change();
        if (
            $change->getFromDB($soluce->input['items_id'])
